...
class EconomyDatabase {
    ...
    public function __construct(string $path) {
        ...
        $this->db->exec("CREATE TABLE IF NOT EXISTS balances (
    ...
    public function setMoney(string $uuid, float $amount): void {
        $stmt = $this->db->prepare("INSERT INTO balances (uuid, money) VALUES (:uuid, :amount)
            ON CONFLICT(uuid) DO UPDATE SET money = :amount");
        $stmt->bindValue(":uuid", $uuid);
    ...
    public function addMoney(string $uuid, float $amount): void {
        $stmt = $this->db->prepare("INSERT INTO balances (uuid, money) VALUES (:uuid, :amount)
            ON CONFLICT(uuid) DO UPDATE SET money = money + :amount");
        $stmt->bindValue(":uuid", $uuid);
        $stmt->bindValue(":amount", $amount);
        $stmt->execute();
    }
    ...
    /**
     * Lấy danh sách top người chơi giàu nhất
     * @return array mảng các mảng ["uuid" => string, "money" => float]
     */
