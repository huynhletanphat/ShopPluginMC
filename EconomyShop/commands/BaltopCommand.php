...
class BaltopCommand extends Command {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool {
        ...
        foreach ($top as $row) {
            ...
            // Lấy tên người chơi từ UUID (nếu online thì dùng getName, nếu không thì tạm hiện UUID rút gọn)
            $player = Server::getInstance()->getPlayerByRawUUID($uuid);
            $name = $player ? $player->getName() : substr($uuid, 0, 8) . "...";
            $sender->sendMessage("§e$i. §f$name: §a" . number_format($money, 0, '.', ',') . " xu");
            $i++;
        }
        if (empty($top)) {
